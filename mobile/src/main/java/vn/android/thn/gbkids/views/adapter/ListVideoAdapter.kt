...
class ListVideoAdapter(private val mContext: Context, var list: ArrayList<RealmVideo>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (getItemViewType(position) == TYPE_DATA) {
            (holder as ListVideoAdapter.ViewHolder).bindData(list.get(position) )
        } else {
            (holder as ListVideoAdapter.ViewHolderLoadMore).onLoadMore()
        }
    }
