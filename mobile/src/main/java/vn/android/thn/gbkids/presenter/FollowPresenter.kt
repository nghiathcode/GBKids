...
import vn.android.thn.gbkids.views.fragment.FollowFragment

class FollowPresenter(view: FollowFragment, mActivity: FragmentActivity?) :
    PresenterBase<FollowFragment>(view, mActivity) , LoadMoreListener {
    ...
    inner class LoadNextListTask : AsyncTask<Int, String, Int>() {
        override fun doInBackground(vararg params: Int?): Int {
            if (view!= null) {
                ...
                var listNew:List<RealmFollow> = realm.where(RealmFollow::class.java).findAll()
                    //Select().from(FollowTable::class.java).execute()
