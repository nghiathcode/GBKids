...
class WatchedFragment : BaseFragment(), ListItemListener {

    var presenter: WatchedPresenter? = null
    ...
    override fun firstInit() {
        if (presenter == null)
            presenter = WatchedPresenter(this, activity!!)
    }
    ...
    fun onListVideo(adapter: HomeAdapter) {
        ...
        if (adapter.list.size == 0){
            updateNoDataText(getString(R.string.no_video_watched))
        } else{
            hideNoData()
        }
    }
