...
import vn.android.thn.gbkids.Movie
...
class CardPresenter : Presenter() {
    ...
    override fun onBindViewHolder(viewHolder: Presenter.ViewHolder, item: Any) {
        val movie = item as VideoTable
        val cardView = viewHolder.view as ImageCardView

        Log.d(TAG, "onBindViewHolder")
        if (movie.toImage() != null) {
            cardView.titleText = movie.title
            cardView.contentText = movie.channelTitle
            cardView.setMainImageDimensions(
                CARD_WIDTH,
                CARD_HEIGHT
            )
            var thumbnails =movie.toImage()
            if (thumbnails!= null) {
                if (GBUtils.isEmpty(movie.urlImage)){
                    if (thumbnails.maxres != null) {
                        movie.urlImage = thumbnails!!.maxres!!.url
                    } else if (thumbnails.high != null) {
                        movie.urlImage = thumbnails!!.high!!.url
                    } else if (thumbnails.medium != null) {
                        movie.urlImage =  thumbnails!!.medium!!.url
                    } else if (thumbnails.standard != null) {
                        movie.urlImage = thumbnails!!.standard!!.url
                    } else if (thumbnails.default != null) {
                        movie.urlImage = thumbnails!!.default!!.url
                    }
                }
